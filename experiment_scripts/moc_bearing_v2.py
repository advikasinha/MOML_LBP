...
def plot_convergence(gen_stats):
    ...
    for ax, vals, lbl, col in zip(
            axes, [bf1, bf2, bf3],
            ["Best Compactness", "Best Connectedness", "Best Simplicity"],
    ...
    fig.suptitle("NSGA-II Convergence (v2)", fontsize=13, fontweight="bold")
    fig.tight_layout()
...
def main(base_dir: str):
    ...
    log.info("=" * 70)
    log.info("  MOC-FS v2  |  pop=%d  gen=%d  fixes: load-norm + balance",
             POP_SIZE, N_GEN)
    log.info("=" * 70)
    ...
    # ── 3. NSGA-II ───────────────────────────────────────────────────────────
    log.info("\n[3] Running NSGA-II  (pop=%d, gen=%d)…", POP_SIZE, N_GEN)
    ...
    for j, nm in enumerate(["Compactness","Connectedness","Simplicity"]):
    ...
    summary = {
        ...
        "nsga2":   {"pop": POP_SIZE, "gen": N_GEN, "time_s": round(opt_t,2)},
        "metrics": {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-dir", default=os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "CWRU-dataset"))
